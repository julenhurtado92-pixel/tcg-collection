(() => {
  ...
  const CONFIG = {
    ...
    minUnitCost: Number(metadata.minUnitPurchaseValue ?? 20),
  };
  ...
  function sortPriceBands(values) {
    const order = ['250€+', '100€ - 249,99€', '50€ - 99,99€', '20€ - 49,99€', '<20€'];
  ...
  function normalizeCard(card, source = 'base') {
    ...
    const normalized = {
      ...
      variant: text(card.variant) || 'Non-foil',
      quantity,
      ...
      total: safeTotal,
      priceBand: text(card.priceBand) || priceBand(unit),
      date,
  ...
  function normalizeTransaction(row, mode = 'historical') {
    ...
    return {
      ...
      variant: text(row.variant) || 'Non-foil',
      quantity,
      unitPurchaseValue: unit,
      totalPurchaseValue: total,
      priceBand: text(row.priceBand) || priceBand(unit),
      date,
  ...
  const baseStockLots = (sourceData.stockLots || []).map((row) => normalizeTransaction(row, 'stock'));
  ...
  function getCards() {
    return [...baseCards, ...localCards];
  }

  function getHistoricalRows() {
    return [...baseTransactions, ...localCards.map((card) => localCardToTransaction(card, 'historical'))];
  }

  function getStockRows() {
    return [...baseStockLots, ...localCards.map((card) => localCardToTransaction(card, 'stock'))];
  }
  ...
  function openCardModal(card) {
    ...
    els.modalBody.innerHTML = `
      <div class="modal-layout">
        <div class="modal-image">${imageHtml(card, 'modal')}</div>
        <div class="modal-content">
          ...
          <div class="hero-actions">
            <button class="primary-action" type="button" data-modal-market="${escapeHtml(card.id)}">Buscar precio de mercado</button>
            <button class="secondary-action" type="button" data-modal-close>Cerrar</button>
          </div>
        </div>
      </div>`;
  ...
  function requestMarketPrice(item, type = 'collection') {
    ...
    if (type === 'wishlist') {
      ...
      const local = localCards.find((card) => card.id === item.id);
      if (local) local.marketPrice = { ...(local.marketPrice || {}), status: 'requested', requestedAt: new Date().toISOString() };
  ...
  function handleWishlistSubmit(event) {
    ...
    const item = {
      ...
      variant: text(values.variant) || 'Non-foil',
  ...
  function exportPortfolioData() {
    ...
    const localStockRows = localCards.map((card) => localCardToTransaction(card, 'stock'));
    const payload = {
      ...sourceData,
      metadata: {
        ...(sourceData.metadata || {}),
        ...
        exportNote: 'Incluye altas guardadas en localStorage desde la web.',
      },
      cards: [...baseCards.map(stripTransientCard), ...localCards.map(stripTransientCard)],
      transactions: [...(sourceData.transactions || []), ...localTransactionRows],
      stockLots: [...(sourceData.stockLots || []), ...localStockRows],
    };
    downloadJson(payload, 'portfolio-data.json');
  }
  ...
  function exportWishlistData() {
    ...
    downloadJson(payload, 'wishlist-data.json');
  }
  ...
  function downloadJson(payload, filename) {
    ...
    showToast(`Archivo preparado: ${filename}`);
  }
