(() => {
  ...
  const sourceMarketData = window.marketPricesData || { metadata: {}, items: [], wishlistItems: [], summary: {} };
  ...
  const CONFIG = {
    ...
    marketPricesKey: 'tcgCollection.marketPrices.v1',
  ...
  const els = {
    ...
    priceResolvedCount: $('#priceResolvedCount'),
    pricePendingCount: $('#pricePendingCount'),
    priceMarketValue: $('#priceMarketValue'),
    priceMarketDelta: $('#priceMarketDelta'),
    priceRecoveryAvg: $('#priceRecoveryAvg'),
    priceTableBody: $('#priceTableBody'),
    marketImportButton: $('#marketImportButton'),
    marketImportInput: $('#marketImportInput'),
    marketExportPending: $('#marketExportPending'),
  ...
  function buildMarketPriceIndex(payload) {
    const index = {};
    const addEntry = (entry) => {
      if (!entry) return;
      const id = text(entry.itemId || entry.id || entry.cardId);
      const market = entry.marketPrice || entry.market || entry;
      if (id && market) index[id] = normalizeMarketPrice(market);
    };
    if (Array.isArray(payload?.items)) payload.items.forEach(addEntry);
    else if (payload?.items && typeof payload.items === 'object') Object.entries(payload.items).forEach(([id, market]) => addEntry({ itemId: id, marketPrice: market }));
    if (Array.isArray(payload?.wishlistItems)) payload.wishlistItems.forEach(addEntry);
    else if (payload?.wishlistItems && typeof payload.wishlistItems === 'object') Object.entries(payload.wishlistItems).forEach(([id, market]) => addEntry({ itemId: id, marketPrice: market }));
    return index;
  }

  function normalizeMarketPrice(market) {
    const source = market || {};
    const selected = roundMoney(source.unit ?? source.selected ?? source.average ?? source.trend ?? source.avg30 ?? source.low ?? 0);
    const status = text(source.status || (selected ? 'resolved' : 'not_requested'));
    return {
      status,
      source: text(source.source || (selected ? 'Cardmarket Data Tables' : '')),
      pricingMode: text(source.pricingMode || ''),
      currency: text(source.currency || 'EUR'),
      unit: selected || null,
      selected: selected || null,
      selectedLabel: text(source.selectedLabel || source.priceLabel || ''),
      sell: source.sell ?? null,
      low: source.low ?? null,
      ...
      liveLow: source.liveLow ?? null,
      liveAvg5: source.liveAvg5 ?? null,
      liveRecommended: source.liveRecommended ?? null,
      liveSampleSize: source.liveSampleSize ?? null,
      liveValidListings: source.liveValidListings ?? null,
      liveRawListings: source.liveRawListings ?? null,
      listingFilter: source.listingFilter || null,
      fallbackPriceGuide: source.fallbackPriceGuide || null,
      liveListings: source.liveListings || null,
      productId: source.productId ?? source.idProduct ?? source.cardmarket?.idProduct ?? null,
      productName: text(source.productName || source.cardmarket?.name || ''),
      productUrl: text(source.productUrl || source.cardmarket?.url || ''),
      productKind: text(source.productKind || source.cardmarket?.kind || ''),
      categoryName: text(source.categoryName || source.cardmarket?.categoryName || ''),
      matchConfidence: source.matchConfidence ?? null,
      matchReason: text(source.matchReason || ''),
      variantFallback: text(source.variantFallback || ''),
      lastUpdated: text(source.lastUpdated || ''),
      marketTotal: source.marketTotal ?? null,
      costTotal: source.costTotal ?? null,
      deltaTotal: source.deltaTotal ?? null,
      recoveryRate: source.recoveryRate ?? null,
      cardmarket: source.cardmarket || null,
      candidates: Array.isArray(source.candidates) ? source.candidates : [],
    };
  }

  function marketDataForItem(id) {
    return importedMarketPrices[text(id)] || fileMarketPriceIndex[text(id)] || null;
  }

  function resolveMarketPrice(card) {
    const base = normalizeMarketPrice(card?.marketPrice || {});
    if (base.status === 'manual') return base;
    const external = marketDataForItem(card?.id);
    if (external) return normalizeMarketPrice({ ...base, ...external });
    return base;
  }

  function marketUnit(item) {
    return roundMoney(item?.marketPrice?.unit ?? item?.marketPrice?.selected ?? 0) || 0;
  }

  function marketTotal(item) {
    const unit = marketUnit(item);
    return roundMoney(unit * toNumber(item?.quantity)) || 0;
  }

  function marketDelta(item) {
    const unit = marketUnit(item);
    if (!unit) return null;
    return roundMoney(marketTotal(item) - toNumber(item.stockCost));
  }

  function marketRecoveryRate(item) {
    const cost = toNumber(item.unitPurchaseValue);
    const unit = marketUnit(item);
  ...
  function marketStatusLabel(status) {
    return {
      not_requested: 'Sin consultar',
      requested: 'Solicitado',
      resolved: 'Resuelto',
      ambiguous: 'Dudoso',
      unmatched: 'Sin match',
      insufficient_sample: 'Sin muestra suficiente',
      unsupported_language: 'Idioma no soportado',
      live_error: 'Error listings',
      live_skipped_limit: 'Saltado live',
      scrape_blocked: 'Scraping bloqueado',
      scrape_blocked_fallback: 'Scraping bloqueado · fallback',
      scrape_error: 'Error scraping',
      scrape_error_fallback: 'Error scraping · fallback',
      scrape_no_product_page: 'Producto no localizado',
      scrape_no_product_page_fallback: 'Producto no localizado · fallback',
      scrape_skipped_limit: 'Saltado scraping',
      scrape_skipped_limit_fallback: 'Saltado scraping · fallback',
      riftbound_image_ambiguous: 'Riftbound versión dudosa',
      riftbound_image_no_code: 'Riftbound sin código de imagen',
      manual: 'Manual',
    }[status] || status || 'Sin consultar';
  }

  function marketDiffClass(value) {
  ...
  function normalizeCard(card, source = 'base') {
    ...
    const normalized = {
      ...
      marketPrice: resolveMarketPrice(card),
    };
  ...
  const fileMarketPriceIndex = buildMarketPriceIndex(sourceMarketData);
  let importedMarketPrices = loadStorage(CONFIG.marketPricesKey, {});
  ...
  const initialWishlist = (initialWishlistData.items || []).map((item) => ({ ...item, source: item.source || 'file', marketPrice: normalizeMarketPrice({ ...(item.marketPrice || {}), ...(marketDataForItem(item.id) || {}) }) }));
  const storedWishlist = loadStorage(CONFIG.wishlistKey, []).map((item) => ({ ...item, marketPrice: normalizeMarketPrice({ ...(item.marketPrice || {}), ...(marketDataForItem(item.id) || {}) }) }));
  ...
  function openCardModal(card) {
    ...
    const market = card.marketPrice || {};
    const marketValue = marketUnit(card);
    const marketValueTotal = marketTotal(card);
    const marketDiff = marketDelta(card);
    const marketRate = marketRecoveryRate(card);
    const marketStatus = market.status || 'not_requested';
    const overrideExists = card.source !== 'local' && Boolean(cardOverrides[card.id]);
    const productLine = market.productId ? `Cardmarket #${escapeHtml(market.productId)}${market.productName ? ` · ${escapeHtml(market.productName)}` : ''}` : 'Sin producto Cardmarket asignado';
    els.modalBody.innerHTML = `
      <div class="modal-layout editable-modal-layout compact-detail-modal">
        <div class="modal-image editable-preview compact-preview">
          ${imageHtml(card, 'modal')}
          <small>${escapeHtml(productLine)}</small>
        </div>
        <div class="modal-content compact-modal-content">
          <div class="modal-title-row">
            ...
            <span class="status-pill status-${escapeHtml(marketStatus)}">${escapeHtml(marketStatusLabel(marketStatus))}</span>
          </div>
          <div class="detail-grid compact-detail-grid">
            <div><span>Unidades</span><strong>${formatDecimal(card.quantity)}</strong></div>
            <div><span>Coste u.</span><strong>${formatCurrency(card.unitPurchaseValue)}</strong></div>
            <div><span>Coste total</span><strong>${formatCurrency(card.stockCost)}</strong></div>
            <div><span>Venta u.</span><strong>${saleUnitValue ? formatCurrency(saleUnitValue) : '—'}</strong></div>
            <div><span>Margen venta</span><strong class="${recoveryClass}">${recoveryLabel}</strong></div>
            <div><span>Mercado u.</span><strong>${marketValue ? formatCurrency(marketValue) : '—'}</strong></div>
            <div><span>Mercado total</span><strong>${marketValue ? formatCurrency(marketValueTotal) : '—'}</strong></div>
            <div><span>Vs coste</span><strong class="${marketDiffClass(marketDiff)}">${marketDiff === null ? '—' : formatCurrency(marketDiff)}</strong></div>
            <div><span>Recup. mercado</span><strong>${marketRate === null ? '—' : formatRatioPercent(marketRate)}</strong></div>
            <div><span>Compras / ventas</span><strong>${formatNumber(purchaseRows)} / ${formatNumber(saleRows)}</strong></div>
          </div>
          <form id="cardEditForm" class="edit-form compact-edit-form" data-card-id="${escapeHtml(card.id)}">
            <div class="form-grid compact-form-grid modal-form-grid">
              ...
              <label><span>Precio venta u.</span><input name="saleUnitValue" type="number" min="0" step="0.01" value="${saleUnitValue ? escapeHtml(formatInputMoney(saleUnitValue)) : ''}" placeholder="0,00" /></label>
              <label><span>Mercado manual u.</span><input name="marketUnitValue" type="number" min="0" step="0.01" value="${market.status === 'manual' && marketValue ? escapeHtml(formatInputMoney(marketValue)) : ''}" placeholder="Opcional" /></label>
              ...
              <label class="full"><span>Notas</span><textarea name="notes" rows="2" placeholder="Estado, cambios manuales, precio de venta…">${escapeHtml(card.notes || '')}</textarea></label>
            </div>
            <div class="hero-actions modal-action-row compact-action-row">
              <button class="primary-action" type="submit">Guardar</button>
              <button class="secondary-action" type="button" data-modal-market="${escapeHtml(card.id)}">Marcar precio</button>
              ${market.productUrl ? `<a class="ghost-button" href="${escapeHtml(market.productUrl)}" target="_blank" rel="noopener">Cardmarket</a>` : ''}
              ${overrideExists ? `<button class="ghost-button danger-text" type="button" data-modal-reset="${escapeHtml(card.id)}">Restaurar</button>` : ''}
  ...
  function handleCardEditSubmit(event) {
    ...
    const marketManualRaw = text(values.marketUnitValue);
    const marketManualUnit = marketManualRaw ? roundMoney(marketManualRaw) : 0;
    ...
    if (marketManualUnit) {
      patch.marketPrice = normalizeMarketPrice({
        ...(card.marketPrice || {}),
        status: 'manual',
        source: 'Manual',
        unit: marketManualUnit,
        selected: marketManualUnit,
        selectedLabel: 'manual',
        marketTotal: roundMoney(marketManualUnit * quantity),
        costTotal: stockCost,
        deltaTotal: roundMoney(marketManualUnit * quantity - stockCost),
        recoveryRate: unitCost ? roundMoney((marketManualUnit / unitCost) * 100) : null,
  ...
  function requestMarketPrice(item, type = 'collection') {
    ...
    renderPrices();
    showToast(`Marcado para actualización: ${name}. Ejecuta el script Cardmarket o importa market-prices.json.`);
  }
  ...
  function renderPrices() {
    ...
    const resolved = cards.filter((card) => marketStatus(card) === 'resolved' || marketStatus(card) === 'manual').length;
    const pending = cards.filter((card) => ['ambiguous', 'unmatched', 'insufficient_sample', 'unsupported_language', 'live_error', 'live_skipped_limit', 'scrape_blocked', 'scrape_error', 'scrape_no_product_page', 'scrape_skipped_limit', 'riftbound_image_ambiguous', 'riftbound_image_no_code', 'requested', 'not_requested'].includes(marketStatus(card))).length;
    const marketValue = cards.reduce((sum, card) => sum + marketTotal(card), 0);
    const costValue = cards.reduce((sum, card) => sum + toNumber(card.stockCost), 0);
    const delta = marketValue ? marketValue - costValue : 0;
    const recovery = marketValue && costValue ? (marketValue / costValue) * 100 : 0;
    if (els.priceItemsCount) els.priceItemsCount.textContent = formatNumber(cards.length);
    if (els.priceResolvedCount) els.priceResolvedCount.textContent = formatNumber(resolved);
    if (els.pricePendingCount) els.pricePendingCount.textContent = formatNumber(pending);
    if (els.priceMarketValue) els.priceMarketValue.textContent = formatCurrency(marketValue);
    if (els.priceMarketDelta) {
      els.priceMarketDelta.textContent = formatCurrency(delta);
      els.priceMarketDelta.className = profitClass(delta);
    }
    if (els.priceRecoveryAvg) els.priceRecoveryAvg.textContent = formatRatioPercent(recovery);
    ...
    els.priceTableBody.innerHTML = cards.map((card) => {
      ...
      const total = marketTotal(card);
      const deltaValue = marketDelta(card);
      const deltaText = deltaValue === null ? '—' : formatCurrency(deltaValue);
      const deltaClass = marketDiffClass(deltaValue);
      const productText = market.productId ? `#${escapeHtml(market.productId)} · ${escapeHtml(market.productName || 'Cardmarket')}` : escapeHtml(market.matchReason || 'Pendiente de matching');
      return `
        <tr data-card-id="${escapeHtml(card.id)}">
          <td class="table-main-cell">
            <button class="table-thumb table-thumb-button" type="button" data-action="detail" data-card-id="${escapeHtml(card.id)}" aria-label="Ver detalle de ${escapeHtml(card.cardName)}">${thumb}</button>
          ...
          <td>${escapeHtml(card.edition)}</td>
          <td class="num">${formatCurrency(card.unitPurchaseValue)}</td>
          <td class="num">${unit ? formatCurrency(unit) : '—'}</td>
          <td class="num">${unit ? formatCurrency(total) : '—'}</td>
          <td class="num ${deltaClass}">${deltaText}</td>
          <td><span class="status-pill status-${escapeHtml(status)}" title="${productText}">${escapeHtml(marketStatusLabel(status))}</span></td>
          <td class="table-actions">
            <button class="small-button" type="button" data-action="market" data-card-id="${escapeHtml(card.id)}">Marcar</button>
            ${market.productUrl ? `<a class="small-button" href="${escapeHtml(market.productUrl)}" target="_blank" rel="noopener">Ver</a>` : ''}
          </td>
        </tr>`;
    }).join('') || '<tr><td colspan="9">No hay elementos para los filtros seleccionados.</td></tr>';
  }

  function importMarketPricesPayload(payload) {
    const index = buildMarketPriceIndex(payload);
    importedMarketPrices = { ...importedMarketPrices, ...index };
    saveStorage(CONFIG.marketPricesKey, importedMarketPrices);
    localCards = localCards.map((card) => normalizeCard({ ...card, marketPrice: normalizeMarketPrice({ ...(card.marketPrice || {}), ...(marketDataForItem(card.id) || {}) }) }, 'local'));
    wishlistItems = wishlistItems.map((item) => ({ ...item, marketPrice: normalizeMarketPrice({ ...(item.marketPrice || {}), ...(marketDataForItem(item.id) || {}) }) }));
    refreshAll();
    const count = Object.keys(index).length;
    showToast(`${formatNumber(count)} precios de mercado importados.`);
  }

  function handleMarketImport(event) {
    ...
    reader.onload = () => {
      try {
        ...
        importMarketPricesPayload(payload);
      } catch (error) {
        console.error(error);
        showToast('No se pudo leer el JSON de precios.');
      } finally {
        event.target.value = '';
      }
    };
  ...
  function exportPendingPriceCsv() {
    const rows = filteredPriceCards().filter((card) => ['not_requested', 'requested', 'ambiguous', 'unmatched'].includes(marketStatus(card)));
    const header = ['itemId', 'tcg', 'name', 'edition', 'language', 'variant', 'quantity', 'unitPurchaseValue', 'status'];
    const csvEscape = (value) => `"${String(value ?? '').replace(/"/g, '""')}"`;
    const lines = [header.join(',')].concat(rows.map((card) => header.map((key) => csvEscape({
      itemId: card.id,
      tcg: card.tcg,
      name: card.cardName,
      edition: card.edition,
    ...
    downloadBlob(lines.join('\n'), 'cardmarket-pending-items.csv', 'text/csv');
    showToast(`${formatNumber(rows.length)} pendientes exportados.`);
  }
  ...
  function bindEvents() {
    ...
    els.marketImportButton?.addEventListener('click', () => els.marketImportInput?.click());
    els.marketImportInput?.addEventListener('change', handleMarketImport);
    els.marketExportPending?.addEventListener('click', exportPendingPriceCsv);
    ...
    const requestAllPrices = () => {
      const rows = filteredPriceCards().filter((card) => !['resolved', 'manual'].includes(marketStatus(card)));
      rows.forEach((card) => {
        persistCardPatch(card, { marketPrice: { ...(card.marketPrice || {}), status: 'requested', requestedAt: new Date().toISOString() } });
      });
      refreshAll();
      showToast(`${formatNumber(rows.length)} elementos marcados. Ejecuta scripts/update_cardmarket_prices.py o importa market-prices.json.`);
    };
