(() => {
  ...
  function renderSummary() {
    ...
    const pendingImages = toNumber(metadata.imageResolution?.magicRowsPendingScryfall);
    ...
    els.metricImages.textContent = formatNumber(pendingImages);
  ...
  function openCardModal(cardId) {
    ...
    if (!card) return;
    const scryfallLink = text(card.scryfall?.uri || card.scryfall?.searchUri);
  ...
  function openMetric(metric) {
    const rows = {
      ...
      quality: [
        ['MTG pendiente Scryfall', formatNumber(metadata.imageResolution?.magicRowsPendingScryfall || 0)],
        ['No MTG sin Scryfall', formatNumber(metadata.imageResolution?.nonMagicRowsWithoutScryfall || 0)],
        ['Revisión campos visibles', formatNumber((sourceData.review || []).length)],
      ],
    }[metric] || [];
