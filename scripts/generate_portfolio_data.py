...
entries by TCG + name + edition + language + variant, and applies the >=20 EUR
...
def extract_variant_label(name: str) -> str:
    labels = []
    if has_foil_marker(name):
        labels.append("Foil")
    else:
        labels.append("Non-foil")
    variant_matches = re.findall(r"\((V\.[^)]*)\)", name or "", flags=re.IGNORECASE)
...
def variant_key(name: str) -> str:
    # Keep grouping conservative for foil/non-foil. Version labels are already retained in the display name.
    return "foil" if has_foil_marker(name) else "nonfoil"
...
def make_transaction(row_index: int, row: list[Any], indexes: dict[str, int]) -> dict[str, Any]:
    ...
    return {
        ...
        "language": language,
        "variant": extract_variant_label(original_name),
        "variantKey": variant_key(original_name),
...
def generate(excel_path: Path, output_dir: Path, min_unit_value: float) -> dict[str, Any]:
    ...
    metadata = {
        ...
        "groupingKey": "TCG + nombre normalizado + edición normalizada + idioma + variante Foil/Non-foil",
...
def main() -> None:
    ...
    parser.add_argument("--min-unit", type=float, default=20.0, help="Coste unitario medio minimo en EUR.")
