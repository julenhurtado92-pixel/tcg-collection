...
PRODUCT_RULES = [
    ...
    {
        "keys": ["starter deck", "one piece film edition"],
        "source": "onepiece_official_representative_image",
        "confidence": "representative_product_page_image",
        "normal": "https://en.onepiece-cardgame.com/images/products/decks/st05/st05-001_d.png",
        "source_url": "https://en.onepiece-cardgame.com/products/decks/st05.php",
        "note": "Official ONE PIECE product page representative card image for ST-05. Used because a deterministic package-only image was not available in the static mapper.",
    },
    {
        "keys": ["starter deck", "absolute justice"],
        "source": "onepiece_official_representative_image",
        "confidence": "representative_product_page_image",
        "normal": "https://en.onepiece-cardgame.com/images/products/decks/st06/st06-001.png",
        "source_url": "https://en.onepiece-cardgame.com/products/decks/st06.php",
        "note": "Official ONE PIECE product page representative card image for ST-06. Used because a deterministic package-only image was not available in the static mapper.",
    },
    {
...
RIFTBOUND_CARD_CODES = {
    ...
    "mirror image": "UNL-200",
    "sabotage": "OGN-156",
    "rift herald": "UNL-179a",
...
MTG_SET_CODES = {
    ...
    "theros beyond death extras": "thb",
    ...
    "kaldheim extras": "khm",
    "ikoria lair of behemoths extras": "iko",
    "dominaria remastered extras": "dmr",
    "streets of new capenna extras": "snc",
    "murders at karlov manor extras": "mkm",
    "the lost caverns of ixalan extras": "lci",
    "lorwyn eclipsed extras": "ecl",
...
def scryfall_named_urls(card_name: str, edition: str) -> Tuple[Dict[str, str], Dict[str, Any]]:
    ...
    extra = scryfall_extra_search(exact, edition, set_code)
    if extra:
        info["extraSearch"] = extra
        info["needsLiveExtraSearch"] = True
    return images, info


def scryfall_extra_search(exact_name: str, edition: str, set_code: str = "") -> Optional[Dict[str, Any]]:
    if "extra" not in norm(edition):
        return None
    query = f"!\"{exact_name}\""
    if set_code:
        query += f" e:{set_code}"
    api_url = f"{SCRYFALL_API}/search?unique=prints&order=set&q={quote_plus(query)}"
    return {
        "apiUrl": api_url,
        "query": query,
        "setCode": set_code,
        "strategy": "live_scryfall_print_search_score_alternative_images",
    }
...
def enrich(data: Dict[str, Any]) -> Tuple[Dict[str, Any], list[Dict[str, Any]]]:
    ...
    for card in data.get("cards", []):
        ...
        if images and info:
            ...
            if source.endswith("product_cdn") or source in {"retailer_thumbnail", "scryfall_representative_set_image", "onepiece_official_representative_image"}:
